...
def ensure_uuid_string_format(fingerprint_id: str | int) -> str:
    """
...
def format_fingerprint_ids(fingerprint_ids: list[str | int]) -> list[str]:
    ...
    for i, (before, after) in enumerate(
        zip(fingerprint_ids, formatted_ids, strict=False)
    ):
        if str(before) != after:
...
def process_candidate_search_message(message_json, publish_result=True):
    ...
    try:
        ...
        logger.error(f"[ERROR] Exception during processing: {e!s}")
        raise
...
def publish_to_queue_b(message: dict):
    ...
    try:
        ...
        logger.error(f"Error publishing to queue {QUEUE_B}: {e!s}")
